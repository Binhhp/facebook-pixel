...
const LayoutPixel = React.lazy(() => import('./components/pixel/LayoutPixel'));
...
const App = () => {
    ...
    const handleSelectedShop = (StepSetup) => {
        setShop({
            ...Shop,
            StepSetup: StepSetup
        });
    ...
    const handleSelectedTabs = (selectedTab) => {
        ...
        childCompRef.current.ReloadComponent(selectedTab);
    }
            ...
            .then(function (response) {
                setTimeout(function () {
                    if (response.data.planNumber == 0 && response.data.countPixel > 0) {
                        ...
                        childCompRef.current.ReloadComponent(moreAppConfig.Tab.SETTING);
                    }
                    else {
                        setpixelEdit(null);
                        setselectedTab(moreAppConfig.Tab.DASHBOARD);
                        childCompRef.current.handleSetOnlyTabChange(moreAppConfig.Tab.DASHBOARD);
                        childCompRef.current.ReloadComponent(moreAppConfig.Tab.DASHBOARD);
                    }
                }, 100);
    ...
    const AppCallbackSelectedTabFunction = (selected) => {
        ...
        childCompRef.current.ReloadComponent(selected);
    }
        ...
        setTimeout(function () {
            axios.get(config.rootLink + '/FrontEnd/GetShop?shop=' + config.shop)
                .then(function (response) {
                    ...
                    setselectedTab(response.data.shop.StepSetup < 2 ? 1 : 0);
                ...
                <>
                    <Suspense fallback={<Loading />}>
                        <LayoutPixel onRef={ref => (childCompRef.current = ref)} shop={Shop} setting={Setting} selectedTab={selectedTab} pixelEdit={pixelEdit} hasRating={hasRating} isCompleteSave={isCompleteSave} feed={Feed}
                            ...
                            onSelectedTabs={handleSelectedTabs}
                            onSelectedShop={handleSelectedShop} />
                    </Suspense>
                                                        ...
                                                        <div className='info'>
                                                            ...
                                                            <p className='mb-10'>On this occasion, we would like
                                                                ...
                                                                <CopyToClipboard text={'HOLIDAY_CODE'}>
                                                                    <a href="#" title='discount code' className='pointer' onClick={() => {
                                                                        settoastSent(<Toast content="The discount code was copied." onDismiss={() => { settoastSent(null) }} duration={4500} />);
                                                                        setStepVote(1);
                                                                    }}><span className='c_DB4F6D'>discount code</span></a>
